...
public class VtaNativePrintPlugin extends Plugin {
    ...
    private String buildPrintBridgeScript() {
        return "(function(){"
            ...
            + "function setNativeInputValue(input,value){var setter=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,'value').set;setter.call(input,value);input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));}"
            + "function normalizeNativeBarcodePayload(payload){try{if(payload==null)return{value:'',format:null,cancelled:false};if(typeof payload==='string'||typeof payload==='number')return{value:String(payload).trim(),format:null,cancelled:false};if(Array.isArray(payload))return normalizeNativeBarcodePayload(payload[0]);if(typeof payload!=='object')return{value:'',format:null,cancelled:false};if(payload.cancelled===true||payload.canceled===true)return{value:'',format:null,cancelled:true};if(Array.isArray(payload.barcodes))return normalizeNativeBarcodePayload(payload.barcodes[0]);var raw=payload.rawValue||payload.displayValue||payload.value||payload.text||'';var value=(typeof raw==='string'||typeof raw==='number')?String(raw).trim():'';var fmt=payload.format;var format=(typeof fmt==='string'||typeof fmt==='number')?String(fmt).trim():null;return{value:value,format:format||null,cancelled:!value};}catch(e){return{value:'',format:null,cancelled:false};}}"
            + "function emitBarcodeScanned(payload){var normalized=normalizeNativeBarcodePayload(payload);window.dispatchEvent(new CustomEvent('vta:barcode-scanned',{detail:{version:1,value:normalized.value,format:normalized.format,cancelled:normalized.cancelled}}));return normalized;}"
            + "async function runNativeBarcodeScan(input){try{var plugin=window.Capacitor&&window.Capacitor.Plugins&&window.Capacitor.Plugins.BarcodeScanner;if(!plugin){alert('Scanner indisponible sur cet appareil.');return;}var permission=await plugin.checkPermissions();if(!permission||permission.camera!=='granted'){permission=await plugin.requestPermissions();}if(!permission||permission.camera!=='granted'){alert('Autorisation camera refusee.');return;}var normalized=emitBarcodeScanned(await plugin.scan());if(normalized.cancelled||!normalized.value){alert('Aucun code-barres detecte.');return;}if(input&&!window.__vtaBarcodeEventHandlerReady){setNativeInputValue(input,normalized.value);var search=document.querySelector('[aria-label=\"Rechercher un produit\"]');if(search)search.click();else input.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));}}catch(e){alert('Scanner indisponible sur cet appareil.');}}"
            + "function patchBarcodeScanner(){if(location.pathname.indexOf('/dashboard/pos')!==0)return;var input=document.querySelector('input[placeholder*=\"scanner\"],input[placeholder*=\"produit\"]');if(!input||document.querySelector('[data-vta-native-scanner]'))return;var button=document.createElement('button');button.type='button';button.textContent='Scanner';button.dataset.vtaNativeScanner='1';button.setAttribute('aria-label','Scanner un code-barres avec la caméra');button.className='h-12 shrink-0 rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-800 shadow-sm';button.addEventListener('click',function(ev){ev.preventDefault();runNativeBarcodeScan(input);},true);input.parentElement&&input.parentElement.appendChild(button);}"
            + "setInterval(patchTicketPage,500);patchTicketPage();"
            + "setInterval(patchAuthenticatedDownloads,500);patchAuthenticatedDownloads();"
            + "setInterval(patchBarcodeScanner,500);patchBarcodeScanner();"
            + "})();";
    }
}
