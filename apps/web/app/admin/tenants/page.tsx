    ...
    <AdminShell>
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.04] shadow-xl shadow-black/10">
        <table className="w-full min-w-[1280px] text-left text-sm">
          <thead className="bg-white/5 text-xs uppercase tracking-wide text-slate-400"><tr><th className="p-4">Entreprise</th><th>Activite</th><th>Pays / Ville</th><th>Contact</th><th>Devise</th><th>Creation</th><th>Derniere connexion</th><th>Version</th><th>Plan</th><th>Statut</th><th>Licences</th><th>Actions</th></tr></thead>
          <tbody>{filtered.map((tenant) => <tr key={tenant.id} className="border-t border-white/10 text-slate-300"><td className="p-4"><div className="flex items-center gap-3"><Logo tenant={tenant} /><div><Link href={`/admin/companies/${tenant.id}`} className="font-black text-cyan-100">{tenant.name}</Link><p className="text-xs text-slate-500">{tenant.slug}</p></div></div></td><td>{tenant.primaryActivity ?? "Non defini"}</td><td>{tenant.country ?? "-"}<p className="text-xs text-slate-500">{tenant.city ?? "-"}</p></td><td>{tenant.phone ?? "-"}<p className="text-xs text-slate-500">{tenant.email ?? "-"}</p></td><td>{tenant.currency ?? "HTG"}</td><td>{new Date(tenant.createdAt).toLocaleDateString("fr-HT")}</td><td>{tenant.lastLoginAt ? new Date(tenant.lastLoginAt).toLocaleString("fr-HT") : "Jamais"}<p className="text-xs text-slate-500">{tenant.lastSeenAt ? `Vu ${new Date(tenant.lastSeenAt).toLocaleString("fr-HT")}` : ""}</p></td><td>{tenant.version}</td><td><b className="text-white">{tenant.plan}</b><p className="text-xs text-slate-500">{tenant.subscriptionStatus}</p></td><td><Status value={tenant.status} /></td><td>{tenant.users} users<p className="text-xs text-slate-500">{tenant.stores} magasins · {tenant.modules.length} modules</p></td><td><div className="grid gap-2"><Link href={`/admin/companies/${tenant.id}`} className="font-bold text-cyan-200">Controler</Link><div className="flex flex-wrap gap-1"><button onClick={() => void updateStatus(tenant.id, "ACTIVE")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Activer</button><button onClick={() => void updateStatus(tenant.id, "PAUSED")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Pause</button><button onClick={() => void updateStatus(tenant.id, "TRIAL")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Essai</button><button onClick={() => void updateStatus(tenant.id, "SUSPENDED")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Suspendre</button><button onClick={() => void updateStatus(tenant.id, "EXPIRED")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Expirer</button></div><div className="flex flex-wrap gap-1"><button onClick={() => void updatePlan(tenant.id, "STARTER")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Starter</button><button onClick={() => void updatePlan(tenant.id, "PRO")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Pro</button><button onClick={() => void updatePlan(tenant.id, "ENTERPRISE")} className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200 hover:bg-white/20">Enterprise</button></div></div></td></tr>)}</tbody>
        </table>
      </div>
    </AdminShell>
  );
