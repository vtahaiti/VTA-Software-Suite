...
export default function AuditLogsPage() {
  ...
  async function load() {
    ...
    if (!response.ok) {
      setError("Impossible de charger le journal audit.");
      setLogs([]);
      setIsLoading(false);
      return;
    }
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Connexions, modifications, suppressions, ventes, stocks, factures, paiements et parametres.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => void exportFile("xlsx")} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold dark:border-slate-700">Excel</button>
            <button onClick={() => void exportFile("pdf")} className="rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white">PDF</button>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 md:grid-cols-3 xl:grid-cols-6">
        <input value={q} onChange={(event) => { setPage(1); setQ(event.target.value); }} placeholder="Recherche instantanee" className="rounded-md border px-3 py-2 dark:border-slate-700 dark:bg-slate-950" />
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-200 px-4 py-3 text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400">{isLoading ? "Chargement..." : `${total} action(s) trouvee(s)`}</div>
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-t border-slate-100 dark:border-slate-800">
                  <td className="p-3">{new Date(log.createdAt).toLocaleString("fr-HT")}</td>
                  <td className="p-3"><UserCell log={log} /></td>
                  <td className="p-3">{log.tenantName ?? "--"}</td>
                  <td className="p-3">{log.entity}</td>
                  <td className="p-3"><span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-200">{log.action}</span></td>
                  <td className="p-3 max-w-sm truncate">{log.message}</td>
                  <td className="p-3">{log.ipAddress ?? "--"}</td>
                  <td className="p-3"><button onClick={() => setSelected(log)} className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold dark:border-slate-700">Voir details</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        {!logs.length ? <div className="p-6 text-sm text-slate-500">Aucune action trouvee pour ces filtres.</div> : null}
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3 text-sm dark:border-slate-800">
          ...
          <div className="flex gap-2">
            <button disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))} className="rounded-md border px-3 py-1.5 disabled:opacity-50 dark:border-slate-700">Precedent</button>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-brand-600">Details audit</p>
            <h2 className="mt-1 text-xl font-bold text-slate-950 dark:text-white">{log.action}</h2>
            <p className="mt-1 text-sm text-slate-500">{new Date(log.createdAt).toLocaleString("fr-HT")} - {log.browser ?? "Navigateur inconnu"} - {log.operatingSystem ?? "Systeme inconnu"}</p>
          </div>
...
function ValueBlock({ title, value }: { title: string; value: unknown }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
      ...
      <pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap text-xs text-slate-600 dark:text-slate-300">{value ? JSON.stringify(value, null, 2) : "Aucune valeur enregistree."}</pre>
    </div>
  );
}
