...
export default function GoodsReceiptsPage() {
  ...
  useEffect(() => { if (purchaseOrderId) void loadOrder(purchaseOrderId); else { setOrder(null); setQuantities({}); } }, [purchaseOrderId]);
  ...
  const receiptItems = useMemo(() => receivableItems
    .map((item) => ({ purchaseOrderItemId: item.id, quantity: Number(quantities[item.id] ?? 0) }))
    .filter((item) => item.quantity > 0), [receivableItems, quantities]);
