...
const statusLabels: Record<string, string> = { DRAFT: "Brouillon", SENT: "Envoyee", APPROVED: "Approuvee", PARTIALLY_RECEIVED: "Partiellement recue", FULLY_RECEIVED: "Recue", RECEIVED: "Recue", CANCELLED: "Annulee" };
...
type Dashboard = { todayPurchases: number; monthPurchases: number; activeSuppliers: number; pendingOrders: number; receiptsToday: number; unpaidInvoices: number };
...
export default function PurchasesPage() {
  ...
  useEffect(() => { const timer = setTimeout(() => loadPurchases(), 250); return () => clearTimeout(timer); }, [search, status, page]);
  ...
  async function loadPurchases() {
    ...
    if (response.ok) { const data = await response.json(); setItems(data.items); setTotal(data.meta.total); }
  }
