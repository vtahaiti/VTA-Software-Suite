...
const statusLabels: Record<string, string> = { DRAFT: "Brouillon", SENT: "Envoyé", ACCEPTED: "Accepté", REJECTED: "Refusé", PAID: "Payé", PARTIALLY_PAID: "Partiellement payé", CANCELLED: "Annulé" };
...
export function SalesDocumentPage({ type, title, eyebrow, createLabel, transformLabel, transformAction }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (response.ok) { const doc = await response.json(); setSelected(doc); setMessage("Document créée avec succes."); setLines([{ productId: "", quantity: 1, unitPrice: 0, discount: 0, tax: 0 }]); setNotes(""); setDiscount(0); await loadDocuments(); return; }
