...
export class ProformasController { constructor(private readonly service: ProformasService) {}
  ...
  @Patch(":id/status") @Permissions("proforma.update") updateStatus(@Req() req: AuthenticatedRequest, @Param("id") id: string, @Body() dto: UpdateSalesDocumentStatusDto) { return this.service.updateStatus(req.user.tenantId, id, dto.status, req.user.id); }
  @Post(":id/payments") @Permissions("payment.create") registerPayment(@Req() req: AuthenticatedRequest, @Param("id") id: string, @Body() dto: CreateInvoicePaymentDto) { return this.service.registerPayment(req.user.tenantId, id, dto, req.user.id); }
