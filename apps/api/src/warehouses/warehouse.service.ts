...
export class WarehouseService { constructor(private readonly prisma:PrismaService){}
 ...
 async findOne(tenantId:string,id:string){const item=await this.prisma.warehouse.findFirst({where:{id,tenantId},include:{stocks:{include:{product:true}}}});if(!item)throw new NotFoundException("Entrepôt introuvable");return item;}
 async create(tenantId:string,dto:CreateWarehouseDto){try{return await this.prisma.warehouse.create({data:{tenantId,name:dto.name,code:dto.code??this.code(dto.name),address:dto.address,isActive:dto.isActive??true}})}catch(error){if(error instanceof PrismaClientKnownRequestError&&error.code==="P2002")throw new ConflictException("Code entrepôt déjà existant");throw error;}}
