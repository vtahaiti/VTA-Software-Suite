...
export class InvoicePrintService {
  ...
  async renderInvoice(tenantId: string, invoiceId: string, format: PaperFormat = "letter") {
    ...
    return this.page(`Facture ${invoice.documentNumber}`, `
      ...
      <div class="top"><div><div class="logo">${this.logoContent(tenant)}</div><strong>${this.escape(this.companyName(tenant))}</strong><br/><span class="muted">${this.escape(this.companyAddress(tenant) || "Adresse non définie")}</span><br/><span class="muted">${this.escape(this.companyPhone(tenant) || "Téléphone non défini")}</span><br/><span class="muted">${this.escape(this.companyEmail(tenant) || "Email non defini")}</span>${this.companyTax(tenant) ? `<br/><span class="muted">NIF: ${this.escape(this.companyTax(tenant))}</span>` : ""}</div><div class="right"><h1>FACTURE</h1><p><strong>${this.escape(invoice.documentNumber)}</strong></p><p>Date: ${this.date(invoice.createdAt)}<br/>Échéance: ${this.date(invoice.issuedAt ?? invoice.createdAt)}</p></div></div>
