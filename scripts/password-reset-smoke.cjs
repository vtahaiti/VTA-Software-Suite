...
const resetDto = fs.readFileSync('apps/api/src/auth/dto/reset-password.dto.ts', 'utf8');
const webAuth = fs.readFileSync('apps/web/lib/auth.ts', 'utf8');
const resetPage = fs.readFileSync('apps/web/app/reset-password/page.tsx', 'utf8');
const passwordHashing = fs.readFileSync('apps/api/src/auth/password-hashing.ts', 'utf8');
...
includes(resetDto, 'newPassword!: string', 'reset DTO must expose newPassword as the canonical field');
includes(webAuth, 'newPassword: string', 'frontend auth client must send newPassword');
includes(resetPage, 'resetPassword({ token, newPassword: password, confirmPassword })', 'reset form must call API with newPassword');
includes(auth, 'const newPassword = dto.newPassword ?? dto.password', 'API must bridge cached legacy payloads safely');
includes(passwordHashing, 'export function hashPassword', 'password hashing must be centralized');
includes(passwordHashing, 'export function comparePassword', 'password comparison must be centralized');
includes(auth, 'password: newPasswordHash', 'reset must persist the newly generated password hash');
includes(auth, 'persistedPasswordWorks', 'reset must verify the persisted hash before returning success');
includes(auth, 'usedAt: new Date()', 'reset token must be consumed after successful password update');
includes(auth, 'id: { not: token.id }', 'other reset tokens must be invalidated after successful reset');
